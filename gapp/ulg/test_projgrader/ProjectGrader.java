...
import gapp.ulg.games.*;
import gapp.ulg.play.*;

import static gapp.ulg.game.board.Board.Dir;
import static gapp.ulg.game.board.GameRuler.*;
import static gapp.ulg.game.board.PieceModel.Species;
import static gapp.ulg.game.util.Probe.*;
import static gapp.ulg.game.util.PlayerGUI.MoveChooser;
import static gapp.ulg.test_projgrader.TestPlayGUI.MParams;
...
/** Grader per il progetto */
public class ProjectGrader {
    private static Path assureExist(Path d) throws IOException {
        if (Files.notExists(d))
            d = Files.createDirectories(d);
        return d;
    }

    private static final Path W_DIR = Paths.get("").toAbsolutePath();   // Working Directory
    private static volatile Path STRATEGY_DIR = null;

    public static void main(String[] args) throws IOException {
        STRATEGY_DIR = assureExist(W_DIR.resolve("strategies"));
        test();
    }
    ...
    private static void test() {
        ...
        ok = gradeTest4(ok);
        ok = gradeTest5(ok);
        System.out.println("Punteggio HW2: "+Math.round(totalScore*10)/10.0);
        ok = gradeTest6(ok);
        System.out.println("Punteggio totale: "+Math.round(totalScore*10)/10.0);
        System.out.flush();
    ...
    private static boolean gradeTest1(boolean ok) {
        if (ok) ok = test_Pos(0.1f, 500);
        if (ok) ok = test_PieceModel(0.1f, 500);
        if (ok) ok = test_BoardDef(0.4f, 500);
        if (ok) ok = test_Action(0.5f, 500);
        if (ok) ok = test_Move(0.3f, 500);
        if (ok) ok = test_GameRulerDef(0.4f, 500);
        if (ok) ok = test_BoardOct(0.5f, 1000);
        if (ok) ok = test_UnmodifiableBoard(0.3f, 500);
        if (ok) ok = test_PlayRandPlayer(0.4f, 1000);
        return ok;
    }
    ...
    private static boolean gradeTest2(boolean ok) {
        if (ok) ok = test_DGameInit(0.1f, 1000, "Othello", 8,8, () -> new Othello(PP[0],PP[1]),
                new int[]{4,4, 3,3}, new int[]{3,4, 4,3});
        if (ok) ok = test_DGame(0.1f, 1000, "Othello", 8,8, "1", G_OTH8_1, () -> new Othello(PP[0],PP[1]));
        if (ok) ok = test_DGame(0.2f, 2000, "Othello", 8,8, "2", G_OTH8_2, () -> new Othello(PP[0],PP[1]));
        if (ok) ok = test_DGame(0.4f, 2000, "Othello", 6,6, "3", G_OTH6, () -> new Othello(-1,6,PP[0],PP[1]));
        if (ok) ok = test_DGameUnMove(0.4f, 2000, "Othello", 8,8, G_OTH8_1, () -> new Othello(PP[0],PP[1]));
        if (ok) ok = test_DGameCopy(0.2f, 1000, "Othello", 8,8, G_OTH8_1, () -> new Othello(PP[0],PP[1]));
        if (ok) ok = test_PlayDGameFRandPlayer(0.2f, 2000, "OthelloFactory", null, OthelloFactory::new);
        if (ok) ok = test_Mix(0.4f, 1000);
        return ok;
    }
    ...
    private static boolean gradeTest3(boolean ok) {
        if (ok) ok = test_DGameInit(0.2f, 200, "Othello", 10,10, () -> new Othello(-1,10,PP[0],PP[1]),
                new int[]{5,5, 4,4}, new int[]{4,5, 5,4});
        if (ok) ok = test_DGame(0.25f, 200, "Othello", 12,12, "4", G_OTH12, () -> new Othello(-1,12,PP[0],PP[1]));
        if (ok) ok = test_DGameUnMove(0.4f, 400, "Othello", 6,6, G_OTH6, () -> new Othello(-1,6,PP[0],PP[1]));
        if (ok) ok = test_DGameCopy(0.4f, 200, "Othello", 12,12, G_OTH12, () -> new Othello(-1,12,PP[0],PP[1]));
        if (ok) ok = test_DGame(0.2f, 200, "Othello from factory", 12,12, "5", G_OTH12,
                () -> {
                    GameFactory<GameRuler<PieceModel<Species>>> gF = new OthelloFactory();
                    gF.params().get(1).set("12x12");
                    gF.setPlayerNames(PP[0],PP[1]);
                    return gF.newGame(); });
        if (ok) ok = test_DGameMechanics(0.9f, 4000, "Othello", new int[] {6,6, 8,8, 10,10, 12,12},
                (t,w,h) -> new Othello(t, w, PP[0],PP[1]));
        if (ok) ok = test_DGameFactoryParams(0.9f, 200, "OthelloFactory",
                (pp,start) -> {
                ...
                OthelloFactory::new);
        if (ok) ok = test_DGameInit(0.25f, 200, "MNKgame", 5,4, () -> new MNKgame(-1,5,4,4,PP[0],PP[1]), null, null);
        if (ok) ok = test_DGame(0.5f, 200, "MNKgame", 3,3, "1", G_333, () -> new MNKgame(-1,3,3,3,PP[0],PP[1]));
        if (ok) ok = test_DGame(1.0f, 200, "MNKgame", 6,5, "2", G_655, () -> new MNKgame(-1,6,5,5,PP[0],PP[1]));
        if (ok) ok = test_DGame(0.5f, 200, "MNKgame", 5,7, "3", G_573, () -> new MNKgame(-1,5,7,3,PP[0],PP[1]));
        if (ok) ok = test_DGameUnMove(0.5f, 200, "MNKgame", 6,5, G_655, () -> new MNKgame(-1,6,5,5,PP[0],PP[1]));
        if (ok) ok = test_DGameCopy(0.5f, 200, "MNKgame", 5,7, G_573, () -> new MNKgame(-1,5,7,3,PP[0],PP[1]));
        if (ok) ok = test_DGame(0.5f, 200, "MNKgame from factory", 6,5, "4", G_655,
                () -> {
                    ...
                    gF.setPlayerNames(PP[0],PP[1]);
                    return gF.newGame(); });
        if (ok) ok = test_DGameMechanics(1.5f, 6000, "MNKgame", new int[] {3,4, 5,3, 7,7, 5,9},
                (t,w,h) -> new MNKgame(t, w, h, Math.max(w,h), PP[0],PP[1]),
                (t,w,h) -> new MNKgame(t, w, h, 3+(Math.max(w,h)-3)/2, PP[0],PP[1]));
        if (ok) ok = test_DGameFactoryParams(1.5f, 200, "MNKgameFactory",
                (pp,start) -> {
    ...
    private static boolean gradeTest4(boolean ok) {
        if (ok) ok = test_EncS(0.5f, 6_000, "Othello8x8 Othello6x6 Othello12x12", new String[] {S_OTH8,S_OTH6,S_OTH12},
                () -> new Othello(PP[0],PP[1]), () -> new Othello(-1,6,PP[0],PP[1]),
                () -> new Othello(-1,12,PP[0],PP[1]));
        if (ok) ok = test_nextSituations(1.0f, 500, "Othello6x6 Othello8x8 Othello12x12", gM -> ((EncS<PieceModel<Species>> s) -> s.decode(gM)),
        ...
        if (ok) ok = test_MCTSPlayer(1.0f, 10_000 /*3_000*/, "MNKgameFactory 3,3,3", 50, 5, false, 50, 70, MNKgameFactory::new,
                new String[0], new Object[0]);
        if (ok) ok = test_MCTSPlayer(1.0f, 45_000 /*10_000*/, "MNKgameFactory 4,4,4", 50, 5, false, 20, 30, MNKgameFactory::new,
                new String[] {"M","N","K"}, new Object[] {4,4,4});
        if (ok) ok = test_OptimalPlayer(0.75f, 1500, "MNKgameFactory 3,3,3", false, 100, false, MNKgameFactory::new, new String[0], new Object[0]);
        if (ok) ok = test_OptimalPlayer(0.75f, 1500, "MNKgameFactory 3,4,3", false, 100, true, MNKgameFactory::new, new String[] {"N"}, new Object[] {4});
        return ok;
    }
    ...
    private static <P> boolean test_MCTSPlayer(float sc, int ms, String msg, int nr, int nt, boolean parallel, int np, int score,
        ...
        return runTest("Test MCTSPlayer con "+msg, sc, ms, () -> {
            try {
                ...
                for (int t = 0 ; t < nt ; t++) {
                    ...
                    if (scr >= score) return new Result();
                    if (scr > maxScore) maxScore = scr;
                    if (scr < minScore) minScore = scr;
                }
                if (maxScore < score) return err("troppo debole: 3*wins - 3*defeats + draw = " + maxScore + " < " + score+"  min: "+minScore);
            } catch (Throwable t) { return handleThrowable(t); }
            return new Result(); });
    }
    ...
    private static boolean gradeTest5(boolean ok) {
        if (ok) ok = test_EncS(0.5f, 4_000, "Camelot", 100, () -> new G_rs_CAM(-1,PP[0],PP[1]).mechanics(), (pp, r) -> {
            Map<Pos,PieceModel<Species>> conf = new HashMap<>();
            for (int i = 0 ; i < 28 ; i++) {
                PieceModel<Species> pm = CAM_PCS.get(i);
                conf.put(pp.get(r.nextInt(pp.size())), pm);
            }
            return new Situation<>(conf, r.nextInt(5) - 2);
        });
        if (ok) ok = test_EncS(0.5f, 6_000, "Breakthrough20x15", 100, () -> new G_rs_BKT(-1,20,15,PP[0],PP[1]).mechanics(), (pp, r) -> {
            Map<Pos,PieceModel<Species>> conf = new HashMap<>();
            for (int i = 0 ; i < 80 ; i++)
                conf.put(pp.get(r.nextInt(pp.size())), (i < 20 ? P_NERO : P_BIANCO));
            return new Situation<>(conf, r.nextInt(5) - 2);
        });
        if (ok) ok = test_nextSituations(0.25f, 1500, "3,3,3-game", gM -> ((EncS<PieceModel<Species>> s) -> s.decode(gM)),
                gM -> (s -> new EncS<>(gM,s)), false, new long[][][] {NS_MNK333}, () -> new MNKgame(-1,3,3,3,PP[0],PP[1]));
        if (ok) ok = test_nextSituations(0.25f, 10_000, "3,4,3-game", gM -> ((EncS<PieceModel<Species>> s) -> s.decode(gM)),
                gM -> (s -> new EncS<>(gM,s)), false, new long[][][] {NS_MNK343}, () -> new MNKgame(-1,3,4,3,PP[0],PP[1]));
        if (ok) ok = test_nextSituations(0.75f, 500, "Breakthrough2x4", gM -> ((EncS<PieceModel<Species>> s) -> s.decode(gM)),
                gM -> (s -> new EncS<>(gM,s)), false, new long[][][] {NS_BT24}, () -> new G_rs_BKT(-1,2,4,PP[0],PP[1]));
        if (ok) ok = test_nextSituations(0.5f, 10_000, "Othello6x6 Parallel", gM -> ((EncS<PieceModel<Species>> s) -> s.decode(gM)),
                gM -> (s -> new EncS<>(gM,s)), true, new long[][][] {NS_OTH6_P}, () -> new Othello(-1,6,PP[0],PP[1]));
        if (ok) ok = test_nextSituations(0.5f, 20_000, "4,4,4-game Parallel", gM -> ((EncS<PieceModel<Species>> s) -> s.decode(gM)),
                gM -> (s -> new EncS<>(gM,s)), true, new long[][][] {NS_MNK444_P}, () -> new MNKgame(-1,4,4,4,PP[0],PP[1]));
        if (ok) ok = test_nextSituations(0.75f, 40_000, "Camelot Parallel", gM -> ((EncS<PieceModel<Species>> s) -> s.decode(gM)),
                gM -> (s -> new EncS<>(gM,s)), true, new long[][][] {NS_CAM_P}, () -> new G_rs_CAM(-1,PP[0],PP[1]));

        // Probabilità di errore inferiore a 1/1_000_000_000
        if (ok) ok = test_MCTSPlayerPlay(0.75f, 1_000, "BreakthroughFactory 2,4", 200, 3, false, 10, 30, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {2,4});
        // Probabilità di errore inferiore a 1/1_000_000_000
        if (ok) ok = test_MCTSPlayerPlay(0.75f, 6_000, "BreakthroughFactory 3,5", 200, 4, false, 10, 24, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {3,5});
        // Probabilità di errore inferiore a 1/1_000_000_000
        if (ok) ok = test_MCTSPlayerPlay(0.75f, 40_000, "OthelloFactory 6x6 (M) Parallel", 100, 5, true, 10, 24, G_rs_OTHFactory::new,
                new String[] {"Board"}, new Object[] {"6x6"});
        // Probabilità di errore inferiore a 1/1_000_000_000
        if (ok) ok = test_MCTSPlayerPlay(0.75f, 8_000, "BreakthroughFactory 3,5 Parallel", 500, 1, true, 60, 126, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {3,5});

        if (ok) ok = test_OptimalPlayerPlay(0.5f, 1000, "BreakthroughFactory 2,4", false, 100, false, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {2,4});
        if (ok) ok = test_OptimalPlayerPlay(0.5f, 6000, "BreakthroughFactory 3,5", false, 100, false, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {3,5});
        if (ok) ok = test_OptimalPlayerPlay(0.5f, 1000, "BreakthroughFactory 2,6", false, 100, true, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {2,6});
        if (ok) ok = test_OptimalPlayerPlay(0.75f, 4_000, "MNKgameFactory 4,3,4 (M) Parallel", true, 100, false, G_rs_MNKFactory::new,
                new String[] {"M","K"}, new Object[] {4,4});
        if (ok) ok = test_OptimalPlayerPlay(0.75f, 60_000, "MNKgameFactory 5,3,5 (M) Parallel", true, 100, false, G_rs_MNKFactory::new,
                new String[] {"M","K"}, new Object[] {5,5});
        return ok;
    }



    private static <P> boolean test_EncS(float sc, int ms, String msg, int n, Supplier<Mechanics<P>> gMS,
                                         BiFunction<List<Pos>,Random,Situation<P>> sGen) {
        return runTest("Test EncS con "+msg, sc, ms, () -> {
            try {
                Random rnd = new Random(67540273);
                List<Situation<P>> sL = new ArrayList<>();
                Mechanics<P> gM = gMS.get();
                for (int t = 0; t < n; t++) {
                    Situation<P> s = sGen.apply(gM.positions, rnd);
                    sL.add(s);
                    EncS<P> es = new EncS<>(gM, s);
                    if (!Objects.equals(s, es.decode(gM))) return err("decode");
                    Map<Pos, P> conf = s.newMap();
                ...
                for (int k = 0; k < np; k++) {
                    ...
                    tt[k] = new Thread(() -> {
                        for (int j = start; j < start + part; j++)
                            arr[j] = new EncS<>(gM, sL.get(j % sL.size()));
                    });
                    tt[k].start();
                }
                for (int k = 0; k < np; k++) {
                ...
                for (MemoryPoolMXBean m : ManagementFactory.getMemoryPoolMXBeans())
                    extra += m.getUsage().getUsed();
                extra /= nt;
                if (extra > 250) return err("codifica non sufficientemente compatta ("+extra+")");
            } catch (Throwable t) { return handleThrowable(t); }
            return new Result(); });
    }

    private static <P> boolean test_MCTSPlayerPlay(float sc, int ms, String msg, int nr, int nt, boolean parallel, int np, int score,
                                                   Supplier<GameFactory<? extends GameRuler<P>>> gFS, String[] pN, Object[] pV) {
        return runTest("Test MCTSPlayer (no Utils.Play) con "+msg, sc, ms, () -> {
            try {
                int maxScore = Integer.MIN_VALUE, minScore = Integer.MAX_VALUE;
                for (int t = 0 ; t < nt ; t++) {
                    Player<P> p = new DummyP<>("R");
                    ...
                    Player<P> mcts = new MCTSPlayer<>("MCTS(" + nr + ")", nr, parallel);
                    int[] rr = {0, 0, 0};
                    for (int i = 0; i < np; i++) rr[play(gF, p, mcts).result()]++;
                    int scr = 3 * rr[2] - 3 * rr[1] + rr[0];
                    if (scr >= score) return new Result();
                    if (scr > maxScore) maxScore = scr;
                    if (scr < minScore) minScore = scr;
    ...
    private static <P> boolean test_OptimalPlayerPlay(float sc, int ms, String msg, boolean parallel, int np, boolean first,
                                                      Supplier<GameFactory<? extends GameRuler<P>>> gFS, String[] pN, Object[] pV) {
        return runTest("Test OptimalPlayer (no Utils.Play) con "+msg, sc, ms, () -> {
            try {
                Player<P> p = new DummyP<>("R");
                GameFactory<? extends GameRuler<P>> gF = gFS.get();
                for (int i = 0 ; i < pN.length ; i++)
                ...
                List<Player<P>> pp = first ? Arrays.asList(op, p) : Arrays.asList(p, op);
                int[] rr = {0,0,0};
                for (int i = 0 ; i < np ; i++) rr[play(gF, pp.get(0), pp.get(1)).result()]++;
                if ((first && rr[1] < np) || (!first && rr[1] > 0)) return err("Non ottimale: "+Arrays.toString(rr));
            } catch (Throwable t) { return handleThrowable(t); }
            return new Result(); });
    }
    ...
    static class Report {
        synchronized void report(String msg) { buffer.append(msg); }

        private synchronized void reset() { buffer.delete(0,buffer.length()); }
    ...
    private static boolean gradeTest6(boolean ok) {
        if (ok) ok = test_PlayerGUI(1.0f, 3_000, "PlayerGUI Move Othello6x6", TestPlayGUI::testMove, G_rs_OTHFactory::new,
                new String[] {"Board"}, new Object[] {"6x6"}, false, 100);
        if (ok) ok = test_PlayerGUI(2.0f, 15_000, "PlayerGUI Move Othello12x12", TestPlayGUI::testMove, G_rs_OTHFactory::new,
                new String[] {"Board"}, new Object[] {"12x12"}, false, 50);
        if (ok) ok = test_PlayerGUI(2.0f, 8_000, "PlayerGUI Move 8,9,7-game", TestPlayGUI::testMove, G_rs_MNKFactory::new,
                new String[] {"M","N","K"}, new Object[] {8,9,7}, false, 50);
        if (ok) ok = test_PlayerGUI(2.0f, 15_000, "PlayerGUI Move Breakthrough10x12", TestPlayGUI::testMove, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {10,12}, false, 50);
        if (ok) ok = test_PlayerGUI(3.0f, 50_000, "PlayerGUI Move Camelot", TestPlayGUI::testMove, G_rs_CAMFactory::new,
                new String[0], new Object[0], false, 30);

        if (ok) ok = test_PlayerGUI(2.0f, 10_000, "PlayerGUI IncludeValidMoves Othello10x10", TestPlayGUI::testIncludeVM, G_rs_OTHFactory::new,
                new String[] {"Board"}, new Object[] {"10x10"}, false, 50);
        if (ok) ok = test_PlayerGUI(2.5f, 10_000, "PlayerGUI IncludeValidMoves 7,9,6-game", TestPlayGUI::testIncludeVM, G_rs_MNKFactory::new,
                new String[] {"M","N","K"}, new Object[] {7,9,6}, false, 50);
        if (ok) ok = test_PlayerGUI(2.5f, 60_000, "PlayerGUI IncludeValidMoves Breakthrough15x13", TestPlayGUI::testIncludeVM, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {15,13}, false, 30);
        if (ok) ok = test_PlayerGUI(3.0f, 300_000, "PlayerGUI IncludeValidMoves Camelot", TestPlayGUI::testIncludeVM, G_rs_CAMFactory::new,
                new String[0], new Object[0], false, 20);

        if (ok) ok = test_PlayerGUI(2.0f, 9_000, "PlayerGUI ValidMoves Othello8x8", TestPlayGUI::testVM, G_rs_OTHFactory::new,
                new String[0], new Object[0], false, 100);
        if (ok) ok = test_PlayerGUI(2.0f, 4_000, "PlayerGUI ValidMoves 5,5,5-game", TestPlayGUI::testVM, G_rs_MNKFactory::new,
                new String[] {"M","N","K"}, new Object[] {5,5,5}, false, 100);
        if (ok) ok = test_PlayerGUI(2.0f, 90_000, "PlayerGUI ValidMoves Breakthrough17x20", TestPlayGUI::testVM, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {17,20}, false, 20);
        if (ok) ok = test_PlayerGUI(4.0f, 170_000, "PlayerGUI ValidMoves Camelot", TestPlayGUI::testVM, G_rs_CAMFactory::new,
                new String[0], new Object[0], false, 20);

        System.out.println("Punteggio parziale: "+Math.round(totalScore*10)/10.0);

        if (ok) ok = test_PlayerGUI(2.0f, 14_000, "PlayerGUI ValidMovesBack Othello12x12", TestPlayGUI::testVMBack, G_rs_OTHFactory::new,
                new String[] {"Board"}, new Object[] {"12x12"}, false, 25);
        if (ok) ok = test_PlayerGUI(2.0f, 7_000, "PlayerGUI ValidMovesBack 7,7,7-game", TestPlayGUI::testVMBack, G_rs_MNKFactory::new,
                new String[] {"M","N","K"}, new Object[] {7,7,7}, false, 50);
        if (ok) ok = test_PlayerGUI(2.0f, 70_000, "PlayerGUI ValidMovesBack Breakthrough16x13", TestPlayGUI::testVMBack, G_rs_BKTFactory::new,
                new String[] {"Width","Height"}, new Object[] {16,13}, false, 20);
        if (ok) ok = test_PlayerGUI(4.0f, 300_000, "PlayerGUI ValidMovesBack Camelot", TestPlayGUI::testVMBack, G_rs_CAMFactory::new,
                new String[0], new Object[0], false, 10);

        System.out.println("Punteggio parziale: "+Math.round(totalScore*10)/10.0);

        if (ok) ok = test_PlayGUI(2.0f, 1_000, "PlayGUI", "Othello", NP, NPV, "Random Player", NP, NPV, "Random Player", NP, NPV, 1, false);
        if (ok) ok = test_PlayGUI(3.0f, 1_000, "PlayGUI", "Othello", new String[] {"Board"}, new Object[] {"12x12"},
                "Random Player", NP, NPV, "Random Player", NP, NPV, 1, false);
        if (ok) ok = test_PlayGUI(4.0f, 2_000, "PlayGUI", "m,n,k-game", new String[] {"M","N","K"}, new Object[] {6,6,6},
                "Random Player", NP, NPV, "Random Player", NP, NPV, 3, false);
        if (ok) ok = test_PlayGUI(3.0f, 6_000, "PlayGUI", "Othello", new String[] {"Board"}, new Object[] {"6x6"},
                "Monte-Carlo Tree Search Player", new String[] {"Rollouts"}, new Object[] {100}, "Random Player", NP, NPV, 1, false);
        if (ok) ok = test_PlayGUI(4.0f, 5_000, "PlayGUI", "m,n,k-game", new String[] {"M","N","K"}, new Object[] {4,4,4},
                "Monte-Carlo Tree Search Player", new String[] {"Rollouts"}, new Object[] {100},
                "Monte-Carlo Tree Search Player", new String[] {"Rollouts"}, new Object[] {100}, 2, false);

        System.out.println("Punteggio parziale: "+Math.round(totalScore*10)/10.0);

        if (ok) ok = test_OptimalPlayerSave(3.0f, 6_000, "MNKgameFactory 4,3,4 (M)", false, 10, false, STRATEGY_DIR,
                G_rs_MNKFactory::new, new String[] {"M","N","K"}, new Object[] {4,3,4});
        if (ok) ok = test_PlayGUI_OP(3.0f, 12_000, "PlayGUI", "m,n,k-game", new String[] {"M","N","K"}, new Object[] {4,3,4},
                "Random Player", NP, NPV, "Optimal Player", NP, NPV, 5, STRATEGY_DIR, 2, false);

        System.out.println("Punteggio parziale: "+Math.round(totalScore*10)/10.0);

        if (ok) ok = test_PlayGUI(2.0f, 1_000, "setGame", null, 0, 100, 200, false);
        if (ok) ok = test_PlayGUI(3.0f, 1_000, "moved", null, 0, 100, 200, false);
        if (ok) ok = test_PlayGUI(3.0f, 4_000, "getMove", "1s", 50, -1, 1500, false);

        return ok;
    }

    private static final int MAX_MOVES = 300;
    private static volatile int rndSeed = 12345;

    private static <P> boolean test_PlayerGUI(float sc, int ms, String msg, BiFunction<MoveChooser<P>,MParams<P>,String> test,
                                              Supplier<GameFactory<? extends GameRuler<P>>> gFS,
                                              String[] pN, Object[] pV, boolean mon, int n) {
        ByteArrayOutputStream output = null;
        ThreadsMonitor monitor = null;
        if (mon) {
            output = new ByteArrayOutputStream();
            monitor = new ThreadsMonitor(1, true, 5, new PrintStream(output), s -> s.startsWith("PlayerGUI"));
        }
        int seed = rndSeed;
        rndSeed++;
        boolean ok = runTest("Test " + msg, sc, ms, () -> {
            try {
                Random rnd = new Random(seed);
                return TestPlayGUI.test_PlayerGUI(gFS, pN, pV, () -> new DummyP<>("R", rnd), test, rnd, MAX_MOVES, n);
            } catch (Throwable t) { return handleThrowable(t); }
        });
        System.out.flush();
        if (mon) {
    ...
    private static boolean test_PlayGUI(float sc, int ms, String msg, String gName, String[] gPN, Object[] gPV, String pName1, String[] pPN1, Object[] pPV1,
                                        String pName2, String[] pPN2, Object[] pPV2, int n, boolean mon) {
        int nt = 0;
        ThreadMXBean tm = null;
        Set<Thread> liveT = null;
        if (mon) {
            REPORT.reset();
            tm = ManagementFactory.getThreadMXBean();
            nt = tm.getThreadCount();   // Numero attuale di thread
            liveT = liveThreads();
            tm.resetPeakThreadCount();
        }
        liveThreads();
        boolean ok = runTest("Test "+msg+" "+toString(gName,gPN,gPV)+"  "+toString(pName1,pPN1,pPV1)+" vs "+toString(pName2,pPN2,pPV2)+"  Partite: "+n, sc, ms, () -> {
            try {
                return TestPlayGUI.test_PlayGUI(gName, gPN, gPV, pName1, pPN1, pPV1, pName2, pPN2, pPV2, n, null, 0);
            } catch (Throwable t) { return handleThrowable(t); }
        });
        System.out.flush();
        if (mon) {
            ...
            System.out.println(String.format(" Threads: peak %d  extra %d %s", tm.getPeakThreadCount() - nt, tm.getThreadCount() - nt, extraThreads(liveT)));
        }
        return ok;
    }

    private static boolean test_PlayGUI(float sc, int ms, String method, String gmTime, long tol, long timeout, long timeNotify, boolean mon) {
        int nt = 0;
        ThreadMXBean tm = null;
        Set<Thread> liveT = null;
        if (mon) {
        ...
        liveThreads();
        boolean ok = runTest("Test PlayGUI Time: "+method, sc, ms, () -> {
            try {
                return TestPlayGUI.test_PlayGUI(method, gmTime, tol, timeout, timeNotify);
            } catch (Throwable t) { return handleThrowable(t); }
        });
        System.out.flush();
        if (mon) {
    ...
    private static <P> boolean test_OptimalPlayerSave(float sc, int ms, String msg, boolean parallel, int np, boolean first, Path dir,
                                                      Supplier<GameFactory<? extends GameRuler<P>>> gFS, String[] pN, Object[] pV) {
        //REPORT.reset();
        liveThreads();
        boolean ok = runTest("Test OptimalPlayer Save con "+msg, sc, ms, () -> {
            try {
                Player<P> p = new DummyP<>("R");
                GameFactory<? extends GameRuler<P>> gF = gFS.get();
                for (int i = 0 ; i < pN.length ; i++)
                    for (Param<?> param : gF.params())
                        if (Objects.equals(param.name(), pN[i]))
                            param.set(pV[i]);
                OptimalPlayerFactory<P> opF = new OptimalPlayerFactory<>();
                if (dir != null) opF.setDir(dir);
                PlayerFactory.Play py = opF.canPlay(gF);
                if (py == PlayerFactory.Play.NO) return err("canPlay ritorna NO");
                long time = System.currentTimeMillis();
                if (py == PlayerFactory.Play.TRY_COMPUTE) {
                    String fail = opF.tryCompute(gF, parallel, null);
                    if (fail != null)
                        return err("tryCompute fallisce " + fail);
                }
                REPORT.report(" tryCompute: "+(System.currentTimeMillis()-time)+"ms ");
                opF = new OptimalPlayerFactory<>();
                if (dir != null) opF.setDir(dir);
                py = opF.canPlay(gF);
                if (py != PlayerFactory.Play.YES) return err("canPlay non ritorna YES");
                time = System.currentTimeMillis();
                Player<P> op = opF.newPlayer(gF, "O");
                REPORT.report(" newPlayer: "+(System.currentTimeMillis()-time)+"ms ");
                time = System.currentTimeMillis();
                List<Player<P>> pp = first ? Arrays.asList(op, p) : Arrays.asList(p, op);
                int[] rr = {0,0,0};
                for (int i = 0 ; i < np ; i++) rr[play(gF, pp.get(0), pp.get(1)).result()]++;
                REPORT.report(" Partite: "+(System.currentTimeMillis()-time)+"ms ");
                if ((first && rr[1] < np) || (!first && rr[1] > 0)) return err("Non ottimale: "+Arrays.toString(rr));
            } catch (Throwable t) { return handleThrowable(t); }
            return new Result(); });
        //String s = REPORT.get();
    ...
    private static boolean test_PlayGUI_OP(float sc, int ms, String msg, String gName, String[] gPN, Object[] gPV, String pName1, String[] pPN1, Object[] pPV1,
                                        String pName2, String[] pPN2, Object[] pPV2, int n, Path dir, int index, boolean mon) {
        int nt = 0;
        ThreadMXBean tm = null;
        Set<Thread> liveT = null;
        if (mon) {
        ...
        boolean ok = runTest("Test "+msg+" "+toString(gName,gPN,gPV)+"  "+toString(pName1,pPN1,pPV1)+" vs "+toString(pName2,pPN2,pPV2)+"  Partite: "+n, sc, ms, () -> {
            try {
                return TestPlayGUI.test_PlayGUI(gName, gPN, gPV, pName1, pPN1, pPV1, pName2, pPN2, pPV2, n, dir, index);
            } catch (Throwable t) { return handleThrowable(t); }
        });
        System.out.flush();
        if (mon) {
    ...
    private static Set<Thread> liveThreads() {
        ...
        return new HashSet<>(tMap.keySet());
    }

    private static String extraThreads(Set<Thread> preT) {
        Set<Thread> currT = liveThreads();
        currT.removeAll(preT);
        String s = "";
        for (Thread t : currT)
            s += (s.isEmpty() ? "[" : ",")+t.getName();
        if (!s.isEmpty()) s += "]";
        return s;
    }
    ...
    private static final List<PieceModel<Species>> CAM_PCS = Arrays.asList(P_NERO,P_NERO,P_NERO,P_NERO,P_NERO,P_NERO,P_NERO,P_NERO,P_NERO,P_NERO,
            K_NERO,K_NERO,K_NERO,K_NERO,P_BIANCO,P_BIANCO,P_BIANCO,P_BIANCO,P_BIANCO,P_BIANCO,P_BIANCO,P_BIANCO,P_BIANCO,P_BIANCO,
            K_BIANCO,K_BIANCO,K_BIANCO,K_BIANCO);
    private static final String[] NP = {};
    private static final Objects[] NPV = {};

    private static String toString(String name, String[] pN, Object[] pV) {
        String s = name;
        for (int i = 0 ; i < pN.length ; i++)
            s += (i == 0 ? "[" : ",")+pN[i]+"="+pV[i]+(i == pN.length-1 ? "]" : "");
        return s;
    }
    ...
    public static <P> GameRuler<P> play(GameFactory<? extends GameRuler<P>> gf, Player<P>...pp) {
        ...
        for (Player<P> p : pp) Objects.requireNonNull(p);
        int np = pp.length;
        if (np < gf.minPlayers() || np > gf.maxPlayers()) throw new IllegalArgumentException();
        String[] names = new String[np];
        for (int i = 0 ; i < np ; i++) names[i] = pp[i].name();
        gf.setPlayerNames(names);
        GameRuler<P> gR = gf.newGame();
        for (int i = 0 ; i < np ; i++) pp[i].setGame(gR.copy());
        while (gR.result() == -1) {
            int i = gR.turn();
            Move<P> m = pp[i-1].getMove();
            gR.move(m);
            for (Player<P> p : pp) p.moved(i, m);
        }
        return gR;
    }
    ...
    private static <P,S> long[] hash(Set<S> ess, Function<S,Situation<P>> dec, ToIntFunction<P> pmH) {
        ...
        for (S es : ess) {
            ...
            long[] sh = new long[2];
            for (int i = 0 ; i < 2 ; i++) sh[i] = ((s.turn > -2 ? s.turn+2 : 5)*(PRIMES[i]-1))%PRIMES[i];
    ...
    private static <P> String testNext(Next<P> next, Situation<P> start, Random rnd) {
        class MS {
            private final Move<P> m;
            private final Situation<P> s;
            private MS(Map.Entry<Move<P>,Situation<P>> e) { this.m = e.getKey(); this.s = e.getValue(); }
        }
        String err = "next";
    ...
    private static final long[][] NS_MNK333 = {{9,9,9,9,671157009,978248121},{72,8,8,72,390779176,55174142},
            {252,7,7,504,669300110,813198616},{756,6,6,1512,724372057,541105463},{1260,5,5,3780,116890271,734509500},
            {1520,0,4,4560,602047107,226159986},{1140,0,3,4092,760704081,234016768},{390,0,2,1328,941398558,690264535},
            {62,0,1,142,16564657,110009836},{0,0,0,0,1000000006,1000000180}};
    private static final long[][] NS_MNK343 = {{12,12,12,12,202186432,797240514},{132,11,11,132,487757140,864680731},
            {660,10,10,1320,869247279,506728916},{2970,9,9,5940,251579835,99025501},{7920,8,8,23760,542008434,897304341},
            {17304,0,7,51912,99545551,656565375},{25956,0,6,97200,95394189,405987041},{26040,0,5,97440,952837224,119954943}};
    private static final long[][] NS_BT24 = {{2,2,2,2,781175164,944047816},{6,3,3,6,590459045,588857455},
            {20,3,4,20,46138280,718722750},{45,0,5,50,441012756,57924999},{106,0,5,114,571813752,109797644},
            {129,0,4,164,843724461,846455232},{155,0,4,214,317874715,782952497},{115,0,4,168,405677466,978277900},
            {87,0,4,126,111223073,218115790},{36,0,3,52,396330179,214287977},{12,0,3,24,431489492,119878856},
            {2,0,2,4,296479188,237856515},{0,0,0,0,1000000006,1000000180}};
    private static final long[][] NS_OTH6_P = {{4,4,4,4,58901177,503927397},{12,3,3,12,892879969,348837457},
            {54,4,5,56,670938805,280906559},{236,2,6,240,506360638,157345838},{1256,3,9,1312,297686371,180520978},
            {6528,1,11,7004,924685086,756222835},{36294,1,12,40968,5802811,634385993},{208212,1,14,235998,20182696,403099200}};
    private static final long[][] NS_MNK444_P = {{16,16,16,16,181788953,983589561},{240,15,15,240,202927024,723802891},
            {1680,14,14,3360,681215834,911687508},{10920,13,13,21840,979175996,968340758},{43680,12,12,131040,839046171,462326583},
            {160160,11,11,480480,496978014,650120554},{400400,10,10,1601600,895130121,730376509}};
    private static final long[][] NS_CAM_P = {{100,112,112,112,629351882,155567243},{10026,112,119,11236,945879240,751824994},
            {568832,1,182,1338564,472754733,422353247}};
...
/*-*****************************************************************************/
    ...
    private static class DummyP<P> implements Player<P> {
        ...
        public DummyP(String name, Random rnd) {
            this.name = name;
            this.rnd = rnd;
            gR = null;
        }
        ...
        public Move<P> getMove() {
            ...
            int r = rnd != null ? rnd.nextInt(vm.size()-1) : (int)Math.floor(Math.random()*(vm.size()-1));
            for (Move<P> m : vm) {
    ...
    static Result handleThrowable(Throwable t) {
        String msg = "";
        boolean fatal = false;
        if (t instanceof Exception) {
            ...
            for (StackTraceElement e : st)
                if (checkHWClass.test(e.toString())) {
                    s = e.toString();
                    break;
                }
            //s = Arrays.toString(st);
            //String s = Arrays.toString(st);
            msg += "Eccezione inattesa"+(!s.isEmpty() ? " "+s+": " : ": ");
        } else {
    ...
    private static boolean runTest(String msg, float score, int ms, Callable<Result> test) {
        ...
        int nt = tmx.getThreadCount();
        if (nt > 100) {
            printlnErr("Troppi thread attivi ("+nt+"), impossibile continuare");
            return false;
        }
        long time = System.currentTimeMillis();
        ...
        Result res = null;
        try {
            res = ms > 0 ? future.get(ms, TimeUnit.MILLISECONDS) : future.get();
        } catch (CancellationException | InterruptedException | TimeoutException | ExecutionException ignored) {}
        future.cancel(true);
        OUTPUT.setThread(Thread.currentThread());
        INPUT.setThread(Thread.currentThread());
        OUTPUT.standardOutput(true);
    ...
    static class Result {
        ...
        Result(String e, boolean f) {
            err = e;
            fatal = f;
        }

        Result() { this(null, false); }
        Result(String e) { this(e, false); }
    }
    ...
    private static class Locker {
        ...
        synchronized void releaseLock() { lock.unlock(); }
    ...
    private static class Output extends OutputStream {
        void standardOutput(boolean std) {
            if (!lock.acquireLock()) return;
            standardOut = std;
            lock.releaseLock();
        }
    ...
    private static class Input extends InputStream {
        void standardInput(boolean std) {
            if (!lock.acquireLock()) return;
            standardIn = std;
            lock.releaseLock();
        }
    ...
    private static final Predicate<String> checkHWClass = s -> s.startsWith("gapp.");
}
