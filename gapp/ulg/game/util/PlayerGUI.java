...
public class PlayerGUI<P> implements Player<P> {
    ...
    /**
     * Il gioco specificato
     */
    private GameRuler<P> gameCurrent;

    /**
     * Il master del gioco specificato
     */
    private Consumer<MoveChooser<P>> master;

    /**
     * L'attuale albero delle mosse
     */
    private volatile Tree<P> moveChooser;
    ...
    public PlayerGUI(String name, Consumer<MoveChooser<P>> master) {
        ...
        if(name == null || master == null)
            throw new NullPointerException("Nessun nome impostato per il giocatore e/o il master non è stato impostato");
    ...
    public void setGame(GameRuler<P> g) {
        ...
        if(gameCurrent != null)
            throw new IllegalArgumentException("Il gioco è gia stato impostato!");
    ...
    public Move<P> getMove() {
        moveChooser = new Tree<>(gameCurrent);

        synchronized (moveChooser) {
            master.accept(moveChooser);

            if (gameCurrent.mechanics().time == -1)

                while (!moveChooser.isMoveHasBeenChoosen())

                    try { moveChooser.wait(); } catch (InterruptedException e) {
                        moveChooser.setInterrupted(true);
                        return null;
                    }
                ...
                while (!hasFinished) {
                    ...
                    try {
                        moveChooser.wait(gameCurrent.mechanics().time);
                ...
                if (!moveChooser.isMoveHasBeenChoosen()) {
                    moveChooser.setInterrupted(true);
        ...
        return moveChooser.getChoosenMove();
    }
