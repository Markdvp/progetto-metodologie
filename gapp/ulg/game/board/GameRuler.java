...
public interface GameRuler<P> {
    ...
    default Set<Move<P>> validMoves(Pos p) {
        Set<Move<P>> insieme = new HashSet<>();
        ...
        if (getBoard().get(p) != null) {
            ...
            for (Move<P> mossa : validMoves()) {

                if (mossa.actions.get(0).kind.equals(Action.Kind.MOVE) || mossa.actions.get(0).kind.equals(Action.Kind.JUMP) || mossa.actions.get(0).kind.equals(Action.Kind.SWAP)) {

                    if (mossa.actions.get(0).pos.contains(p)) {
            ...
            for (Move<P> mossa : validMoves()) {
                ...
                if (mossa.actions.get(0).kind.equals(Action.Kind.ADD)) {
